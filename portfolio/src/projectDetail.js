  ...
  troubleShooting: [
    ...
    },
    {
      ...
      approach: [
        ...
        '또한, 더욱 정확한 시간 계산을 위해 호출 간격을 500ms로 줄였습니다. 하지만, 이 과정에서 useState로 상태를 관리하게 되면 렌더링이 과도하게 발생하여 성능 저하의 우려가 있었습니다.',
        '따라서 변수를 생성하여 실제 시간 값은 렌더링에 영향을 주지 않는 방식으로 관리하되, 1초 차이가 날 경우에만 useState를 통해 상태를 업데이트하고 화면에 반영하도록 하여 성능을 고려했습니다.',
      ],
      wil: [
